...
→ Grab 锁定选点，继续追踪；可拖动，也可原地直接张开两指
→ 稳住手机并保持对准目标，张开两指（Release）后拍高清照并复制
→ 触摸调试仍在按下时拍照，松开触点后使用已保存的照片复制
→ Clipboard
...
无光标回归修复：默认关闭未经目标手机校准的置信度硬门槛；空/短置信度数组不再伪装成 0 分，有效手部坐标恢复驱动光标。原生回调检查手部类型与完整关键点，避免非手部对象打断处理。`DEBUG_MODE=true` 时自动输出 `WORLD_CLIPBOARD_HAND` 简短诊断（会话就绪、点数、score、接受状态），不需要手动启用大型 Debug 面板。手机必须关闭旧预览，使用重新编译后的版本复测。

Pinch 不触发修复：原 120ms 观测间隔限制会阻止 5–8Hz 手部回调确认捏合，默认档改为 200ms；同一位置/掌形连续追踪时，native ID 改变不再无条件重置捏合。仍保留两次有效观测、30ms 确认、距离滞回与丢失保护。新增捏合确认/边缘提示和 ID、间隔、两指尖诊断，详见 [修复记录](docs/gesture-round2.md)。重新编译生成预览后，同一只手的拇指与食指指尖相碰，保持到“已抓住”，再自然张开。

松手不复制修复（2026-09-13）：真实手势现在先确认 Release，再把相机交给原生 `takePhoto`，避免 Grab 拍照时暂停追踪、漏掉松手。开关为 `cutout-config.ts` 的 `CAPTURE_HAND_PHOTO_ON_RELEASE=true`。点和候选框在 Grab 锁定，高清 RGB 来自松手后的照片，所以全过程保持手机对准目标；不必拖动。相机恢复等待视图更新完成再启动 VisionKit。跟踪丢失仍只取消，不冒充 Release；重复松手只复制一次。触摸、分割算法及捏合阈值未改。新增连续三次复制、恢复延迟、拍照失败重试、取消等自动化回归；实际手机成功率仍需重新编译预览后验证。

2026-09-13 抓取超时修复：电脑 WLAN 地址已变为 `192.168.67.18`，`miniprogram/config.ts` 已同步；换网络后仍需更新该地址。最终抠图请求预算改为 25 秒，悬停选取为 4 秒，统一在 `miniprogram/vision/cutout-config.ts` 配置，同时传给 `wx.uploadFile.timeout` 与 JS 兜底定时器。超时会中止上传、忽略迟到结果，保留现有失败恢复和下一次抓取；不自动重复上传高清照片。提示包含超时阶段，debug 请求成功时输出往返耗时。电脑后端已重新启动并通过局域网 `/api/health`；手机需要与电脑同网，关闭旧小程序后重新编译并生成新预览。针对请求预算、超时中止/迟到回调与重试的回归测试及小程序 TypeScript 检查通过；电脑合成 HTTP 检查不代表手机延迟实测。
